  ...
  <div class="content-index">
    <article class="main-content page-page">
      <div class="post-header">
        ...
        <div class="post-data">
          <time>{{ problem.createTime }}</time>
          in <a href="#" v-if="problem.category != null">{{ problem.category.name }}</a>
          题目类型：<span>{{ typeChange[problem.type] }}</span>
          |题目难度：<span>{{ difficultyLevels[problem.difficult] }}</span>
          |完成情况：<span>{{ finishedChange[problem.finished] }}</span>
          |掌握程度：<span>{{ starsChange[problem.stars] }}</span>
        </div>
      </div>
      <div class="post-content" itemprop="problemBody">
        <p v-if="problem.tags != null" class="post-tags">
...
<script>
import { commentAdd, getByProblem } from '@/api/comment'
...
export default {
  ...
  methods: {
    ...
    fetchCommentData() {
      getByProblem(this.$route.params.id).then(res => {
        if (res.code === 200) {
          this.list = res.data
        } else {
